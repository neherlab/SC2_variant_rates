...
\response{I think the primary issue here is that there rates are per gene, and not per site. The objective here was to show how the total non-synonymous change is broken down by gene. I have now split `ORF1ab' TODO!! }
...
\response{Thank you for this suggestion. I have included these numbers in Table 1. TODO}
...
\response{Yes, this was poorly worded. What I mean to say is that when there are many lineages, time since emergence can estimated accurately from the divergence of many independent evolutionary trajectories. When there is a single or a few large subclades, estimates of emergence times are more noisy. I have made an effort to reword this. }

\refc{What are the advantages of this method over using BEAST to estimate rates?}

\response{The primary advantages are (i) being able to use more data, and (ii) being more robust to problematic or misdated sequences. }
...
\response{Thank you for this suggestion, I have added this for panels A and B. The number of synonymous sites is tricky to define (2-fold, 4-fold, etc). }

\refc{Line 26 first column page 8 -- 1 x 10-3 would be clearer}

\response{$1\times 10^{-3}$ vs $10^{-3}$? I am not sure this helps.}

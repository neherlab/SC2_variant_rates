...
if __name__=="__main__":
 ...
 #   filtered_data["outlier"] = filtered_data.residuals.apply(lambda x: np.abs(x)>5*iqd)
    ...
    axs[0,0].plot(x, regression.intercept + regression.slope*x + tolerance(x), lw=4)

    axs[0,1].set_title(f'amino acid differences', fontsize=fs*1.2)
    sns.histplot(x=filtered_data.numdate[ind], y=np.minimum(ymax*1.5, filtered_data.aaDivergence[ind]), bins=bins, ax=axs[0,1])
    axs[0,1].plot(x, regression_clean_aa["intercept"] + regression_clean_aa["slope"]*x, lw=4, label=f"slope = {regression_clean_aa['slope']:1.1f} subs/year")
    axs[0,1].errorbar(regression_clean_aa["date"], regression_clean_aa["mean"], regression_clean_aa["stderr"])

    axs[0,2].set_title(f'synonymous differences', fontsize=fs*1.2)
    sns.histplot(x=filtered_data.numdate[ind], y=np.minimum(ymax*1.5, filtered_data.synDivergence[ind]), bins=bins, ax=axs[0,2])
